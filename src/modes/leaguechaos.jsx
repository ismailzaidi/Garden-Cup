...
// Shared with league.jsx on purpose: defaultConfig() flattens every mode's
// config into one object, so two modes offering the same key must agree on
// its default or a mode switch shows the wrong button pre-selected.
...
export default {
  ...
  summary: ({ players, config }) => {
    ...
    return (
      <>
        ...
        then the top 2 meet in a <b style={{ color: C.ink }}>best-of-{finalLegs} chaos final</b> where every leg is dealt a random silly rule.
      </>
    );
  },
  ...
  subtitle: ({ config }) => {
    const legCount = config.legCount ?? DEFAULT_LEGS;
    const finalLegs = config.finalLegs ?? DEFAULT_FINAL_LEGS;
    return `${legCount} LEG${legCount > 1 ? "S" : ""} · TOP 2 · BEST-OF-${finalLegs} CHAOS FINAL`;
  },
  ...
  tabs: ({ matches, config }) => {
    ...
    return [
      ...
      { key: "final", label: finalMatches.length ? `Chaos Final · ${finalPlayedCount}/${config.finalLegs ?? DEFAULT_FINAL_LEGS}` : "Chaos Final" },
    ];
  },
  advance,
